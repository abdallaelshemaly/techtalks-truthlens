...
class FaceDataset(Dataset):
    def __init__(self, real_dir, fake_dir, transform=None):
        ...
        print(
            f"✓ Dataset: {len(self.images)} images (Real: {real_count}, Fake: {fake_count})"
        )
...
def train_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for inputs, labels in pbar:
        ...
        pbar.set_postfix(
            loss=f"{running_loss/len(dataloader):.4f}", acc=f"{100.*correct/total:.1f}%"
        )
...
def main():
    ...
    REAL_DIR = "data/processed/real"  # FIXED: relative path
    FAKE_DIR = "data/processed/fake"  # FIXED: relative path
